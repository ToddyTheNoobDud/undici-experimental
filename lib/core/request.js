...
class Request {
  constructor (origin, {
    ...
    this.validateInputs(path, method, upgrade, headersTimeout, bodyTimeout, reset, expectContinue, throwOnError)
    
    this.headersTimeout = headersTimeout
    this.bodyTimeout = bodyTimeout
    this.method = method
    this.abort = null
    this.body = body ? this.processBody(body) : null
  ...
  validateInputs (path, method, upgrade, headersTimeout, bodyTimeout, reset, expectContinue, throwOnError) {
...
function processHeader (request, key, val) {
  ...
  let headerName = headerNameLowerCasedRecord[key] || key.toLowerCase();
  if (!headerNameLowerCasedRecord[headerName] && !isValidHTTPToken(headerName)) {
    throw new InvalidArgumentError('invalid header key')
  }

  if (Array.isArray(val)) {
    const arr = val.reduce((acc, v) => {
      if (typeof v === 'string' && isValidHeaderValue(v)) {
        acc.push(v);
      } else if (v === null) {
        acc.push('');
      } else {
    ...
    val = arr;
  } else if (typeof val === 'string' && !isValidHeaderValue(val)) {
    throw new InvalidArgumentError(`invalid ${key} header`)
  } else {
    val = val === null ? '' : `${val}`;
  }

  assignHeader(request, headerName, val, key);
}

function assignHeader(request, headerName, val, originalKey) {
  if (request.host === null && headerName === 'host') {
    ...
    request.headers.push(originalKey, val);
  } else if (['transfer-encoding', 'keep-alive', 'upgrade'].includes(headerName)) {
    throw new InvalidArgumentError(`invalid ${headerName} header`);
  } else if (headerName === 'connection') {
    const value = typeof val === 'string' ? val.toLowerCase() : null;
    if (!['close', 'keep-alive'].includes(value)) {
